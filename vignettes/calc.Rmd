  ...
  spp <- tbl(con, "spp") %>% 
    mutate(
      groups01 = case_when(
        ...
        Class == "Cephalopoda" & !Order %in% c("Myopsida","Oegopsida","Teuthida") ~ "Non-squid cephalopods",
...
r_pu_id <- raster(here("../bbnj/inst/data/pu_id.tif"))
cells_bbnj <- tibble(
...
for (i in seq_along(tbls_mdls)){ # i=1 # i=2
  ...
  for (col_grps in cols_grps){ # col_grps = cols_grps[2] # col_grps <- "groups00"
    ...
    for (group in groups){ # group = groups[9]
      
      grp <- group_to_grp(group) 
      message(glue("    grp: {grp} -- {Sys.time()}"))
      
      for (product in products){ # product = products[2]
        ...
        if (product == "rli"){
          calc_rli(con, tif, col_grps, group, tbl_spp_cells, col_spp, spp_prob_threshold)}
